...
class searchRotated {
	public static int binary_search(int[] arr, int st, int end, int key) {
		...
		if (arr[st] <= arr[mid] && key <= arr[mid] && key >= arr[st]) {
			return binary_search(arr, st, mid - 1, key);  // to move into left side
		} else if (arr[mid] <= arr[end] && key >= arr[mid] && key <= arr[end]) {
			return binary_search(arr, mid + 1, end, key);  // to move into right side
		} else if (arr[st] >= arr[mid]) {
			return binary_search(arr, st, mid - 1, key);  // to move into left side in unsorted array
		} else if (arr[end] <= arr[mid]) {
			return binary_search(arr, mid + 1, end, key); // to move into right side in unsorted array
		}
