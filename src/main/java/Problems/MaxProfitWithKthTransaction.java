...
class MaxProfitWithKthTransaction {
...
/*Time complexity of above solution is O(kn) and space complexity is O(nk). 
 * Space complexity can further be reduced to O(n) as we uses the result from last transaction.
 *  But to make the article easily readable, we have used O(kn) space.*/
} 